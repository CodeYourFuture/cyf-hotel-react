...
var index = function (tag, cls, objs) {
  ...
  function Styled(props, context) {
    ...
    push(finalObjs, emotionUtils.reduce(newSpec, function (accum, spec) {
      push(accum, spec.objs);
      if (spec.content) {
        accum.push(spec.content.apply(null, emotionUtils.map(spec.vars, getValue)));
      }
      accum.push(spec.cls);
      return accum;
    }, []));
    ...
    var className = emotion.css(emotionUtils.map(finalObjs, getValue));

    return react.createElement(localTag, emotionUtils.omit(emotionUtils.assign({}, props, {
      ref: props.innerRef,
      className: className
    }), omitFn));
  }

...
var StyleSheet = function () {
  ...
  StyleSheet.prototype.flush = function flush() {
    if (isBrowser) {
      emotionUtils.forEach(this.tags, function (tag) {
...
function buildStyles(objs) {
  ...
  emotionUtils.forEach(objs, function (cls) {
...
function insertRawRule(css) {
  var spec = {
    id: emotionUtils.hashString(css),
    css: css,
    type: 'raw'
  };
  ...
  if (!exports.inserted[spec.id]) {
    sheet.insert(spec.css);
    exports.inserted[spec.id] = true;
  }
}
...
function injectGlobal(objs, vars, content) {
  ...
  emotionUtils.forEach(deconstructedStyleToCSS('', deconstruct(style)), function (rule) {
    insertRawRule(rule);
  });
}

function fontFace(objs, vars, content) {
  var combined = emotionUtils.reduce(content ? objs.concat(content.apply(null, vars)) : objs, function (accum, item, i) {
    return emotionUtils.assign(accum, item);
  }, {});

  insertRawRule('@font-face{' + emotionUtils.createMarkupForStyles(combined) + '}');
}

function insertKeyframe(spec) {
  if (!exports.inserted[spec.id]) {
    var inner = emotionUtils.map(emotionUtils.keys(spec.keyframes), function (kf) {
      return kf + ' {' + emotionUtils.createMarkupForStyles(spec.keyframes[kf]) + '}';
    }).join('');

    emotionUtils.forEach(['-webkit-', ''], function (prefix) {
...
function keyframes(objs, vars, content) {
  ...
  var spec = {
    id: emotionUtils.hashObject(kfs),
    type: 'keyframes',
    name: name,
    keyframes: kfs
  };
...
function hydrate(ids) {
  emotionUtils.forEach(ids, function (id) {
    return exports.inserted[id] = true;
  });
}
...
function objStyle() {
  ...
  rules = emotionUtils.clean(rules);
  if (!rules) {
    return nullrule;
  }
...
function _css(rules) {
  ...
  var spec = {
    id: emotionUtils.hashObject(style),
    style: style,
    type: 'css'
  };
  return toRule(spec);
...
function isLikeRule(rule) {
  var ruleKeys = emotionUtils.keys(rule);
...
function selector(parentSelector) {
  ...
  var x = emotionUtils.map(path.split(','), function (x) {
...
function deconstruct(style) {
  ...
  emotionUtils.forEach(emotionUtils.keys(style), function (key) {
    if (key.indexOf('&') >= 0) {
      selects = selects || {};
...
function deconstructedStyleToCSS(id, style) {
  ...
  if (plain) {
    css.push(selector(id) + '{' + emotionUtils.createMarkupForStyles(plain) + '}');
  }
  if (selects) {
    emotionUtils.forEach(emotionUtils.keys(selects), function (key) {
      return css.push(selector(id, key) + '{' + emotionUtils.createMarkupForStyles(selects[key]) + '}');
    });
  }
  if (medias) {
    emotionUtils.forEach(emotionUtils.keys(medias), function (key) {
      return css.push(key + '{' + deconstructedStyleToCSS(id, medias[key]).join('') + '}');
    });
  }
  if (supports) {
    emotionUtils.forEach(emotionUtils.keys(supports), function (key) {
...
function insert(spec) {
  if (!exports.inserted[spec.id]) {
    ...
    emotionUtils.map(deconstructedStyleToCSS('.css-' + spec.id, deconstructed), function (cssRule) {
...
function joinSelectors(a, b) {
  var as = emotionUtils.map(a.split(','), function (a) {
    return !(a.indexOf('&') >= 0) ? '&' + a : a;
  });
  var bs = emotionUtils.map(b.split(','), function (b) {
    return !(b.indexOf('&') >= 0) ? '&' + b : b;
  });

  return emotionUtils.reduce(bs, function (arr, b) {
    return arr.concat(emotionUtils.map(as, function (a) {
...
function flatten(inArr) {
  var arr = [];
  emotionUtils.forEach(inArr, function (val) {
...
function build(dest, _ref) {
  ...
  src = flatten(src);
  emotionUtils.forEach(src, function (_src) {
    if (isLikeRule(_src)) {
    ...
    _src = emotionUtils.clean(_src);
    if (_src && _src.composes) {
      build(dest, { selector: selector, mq: mq, supp: supp, src: _src.composes });
    }

    emotionUtils.forEach(emotionUtils.keys(_src || {}), function (key) {
      // replace fragments
      if (isFragment(key)) {
